...
#     NREP=100 NSNP=2000 bash src/simulation/experiments/run_power_gain.sh
#
...
run_power_gain_grid() {
    ...
    if [[ "${RUN_VISUALS}" == "1" ]]; then
        run_cmd "${PYTHON}" src/simulation/plot_results.py \
            --metric power \
            --runname "${TRACEC_RUNNAME}" \
            --row h2sq \
            --col gc \
        ...
        run_cmd "${PYTHON}" src/simulation/experiments/plot_power_gain.py \
            --base_path "${OUT_DIR}" \
