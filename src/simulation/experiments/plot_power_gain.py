...
from plot_results import color_map
...
def parse_args():
    ...
    parser.add_argument(
        "--base_path",
        type=Path,
        default=Path("bench/result/power_gain"),
...
def read_result_df(base_path, runname, omega):
    ...
    raise FileNotFoundError(
        ...
        "Run plot_results.py aggregation first."
    )
...
def plot_figure(tracec_summary, tracecb_summary, output_prefix):
    ...
    plot_panel(
        ...
        title="Comparison between traceC and original study",
        x_label=r"$N_2$",
        legend_title=r"$\rho$",
    )
    plot_panel(
        ...
        title="Comparison between traceCB and traceC",
        x_label=r"$\pi$",
        legend_title=r"$N_t$",
    )
