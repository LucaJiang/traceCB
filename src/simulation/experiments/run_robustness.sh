...
# Supplementary robustness grids using simulate_robustness.py.
#
# Usage:
#   bash src/simulation/experiments/run_robustness.sh
#
# Useful server overrides:
#   SIM_DATA_DIR=/path/to/simulation/data OUT_DIR=/path/to/result \
#     NREP=100 NSNP=2000 bash src/simulation/experiments/run_robustness.sh
#
...
run_robustness_grid() {
    ...
    local sim_args=(
        "${PYTHON}" src/simulation/experiments/simulate_robustness.py
        --pop1_geno "${POP1_GENO}"
        --pop2_geno "${POP2_GENO}"
        --runname "${runname}"
        "$@"
