...
#     NREP=100 NSNP=2000 bash src/simulation/experiments/run_tracecb2.sh
#
...
run_tracecb2_grid() {
    ...
    local sim_args=(
        "${PYTHON}" src/simulation/experiments/simulate_tracecb2.py
        --pop1_geno "${POP1_GENO}"
        --pop2_geno "${POP2_GENO}"
        --runname "${runname}"
        "$@"
...
plot_tracecb2_grid() {
    ...
    local visual_args=(
        src/simulation/experiments/plot_tracecb2.py
        --metric "${metric}"
        --runname "${runname}"
