...
for target_pathway in pathways_gene_dict.keys():
                ...
                for gene in result_df.index:
                    if gene in df["gene"].values:
                        ...
                        if gene_pval < egene_pval_threshold:
                            ...
                            result_df.at[gene, "geneid"] = df[df["gene"] == gene]["phenotype_id"].values[0]
                        # else:
    ...
    def get_egene_ours():
        result_df = pd.DataFrame(0, index=oasis_egene_df_1e5.index, columns=["geneid"]+meta_data["QTDids"])
    ...
    ax = sns.heatmap(
        gene_qtd_df, cmap=cmap, norm=norm, cbar=False, linewidths=6, square=True,
    ...
    for celltype, (x_start, width) in celltype_ranges.items():
        ...
        ax.text(
            ...
            color="#000000", fontsize=14, ha="center", va="bottom"
        )
    ...
    legend_elements = [
        ...
        SymbolPatch(symbol="+", facecolor="white", edgecolor="white", label="Replicated in OASIS (p<5e-3)"),
        SymbolPatch(symbol="++", facecolor="white", edgecolor="white", label="Replicated in OASIS (p<1e-5)"),
    ]
