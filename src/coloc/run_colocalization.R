...
if (length(args) != 4) {
    stop("Usage: Rscript run_colocalization.R <loci.csv> <gwas.csv> <eqtl_dir> <output.csv>")
}
loci.path <- args[1]
gwas.path <- args[2]
eqtl.path <- args[3]
...
for (i in 1:n.loci) {
    ...
    eqtl.file <- file.path(
        eqtl.path,
        paste0("chr", loci.data$chr),
        paste0(loci.data$gene, ".parquet")
    )
    if (!file.exists(eqtl.file)) {
        next
    }
    eqtl.gene <- as.data.table(arrow::read_parquet(eqtl.file))
